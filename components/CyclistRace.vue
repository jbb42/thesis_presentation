...
function restartSimulation() {
  // 1. Instantly freeze doubling counters to prevent accidental trigger cascades
  redDoublings.value = 0
  blueDoublings.value = 0
  currentAvgSpeed.value = 60
  id = 0
  
  // 2. Clear out all physics and graph arrays
  cyclists.value = []
  plotHistory.value = []

  // 3. Force Vue to completely flush the DOM layout before spawning new riders
  nextTick().then(() => {
    // Only spawn the pristine 2 initial frontrunners once the board is mathematically blank
...
function spawn(color, speed, leaderX, nextMarkIndex) {
  ...
  // ---> ADJUST THIS BASE VALUE (e.g., changed from -2 to 8) TO PUSH THEM DOWN <---
...
onMounted(() => {
  nextTick().then(() => {
    ...
    function loop(now) {
      updateSimulation((now - last) / 1000)
      last = now
